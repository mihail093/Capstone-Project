...
export default function PlantCommentAreaComponent({ plantId }) {
  const { user } = useAuth();
  ...
  const fetchComments = async () => {
    setIsLoading(true);
    try {
      const response = await plantApi.getComments(plantId);
  ...
  const calculateAverageReviews = () => {
    ...
    const averageRatings = sumRatings / comments.length;
    setAverageReviews(averageRatings);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await plantApi.addComment(plantId, { text: comment, rating });
      setComment('');
      setRating(0);
      fetchComments();
    } catch (err) {
      ...
      {user ? (
        <>
          <form onSubmit={handleSubmit} className="flex max-w-4xl flex-col gap-6 px-8 py-6">
            <div>
              ...
              <Textarea
                id='comment'
                name='comment'
                placeholder="Lascia un commento per questa pianta"
                required 
                shadow
                value={comment}
          ...
          <div className="mt-8">
            ...
            <p>Valutazione media: {averageReviews.toFixed(1)}/5</p>
            {comments.length > 0 ? (
